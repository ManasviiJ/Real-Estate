...
mycon = mys.connect(host="localhost", user="root", passwd="root", database="re_estate",buffered=True)
cursor = mycon.cursor()
...
def create_profile(the_username):                               ## prev user data is shown ## To be used in forgot password button as well
    ...
    if role == "Tenant":
        ...
        for widget in sidebar.winfo_children():
            if isinstance(widget, tb.Button) and widget.cget("text") in ["POST\nYOUR\nPROPERTY", "My\nProperties"]:
                widget.destroy()
               
        # Add tenant dashboard button
        tb.Button(sidebar, text="Tenant\nDashboard", bootstyle=SUCCESS,
                 command=tenant_dashboard_open).grid(row=1, column=0, pady=10)
